...
impl <'a> GraphBuilder<'a> {
	...
	pub fn build(mut self, no_play: bool, output_mode: &PlayerOutput) -> ModdlResult<(Vec<MachineSpec>, RegisterSettingsTree)> {
		...
		// let track_defs = self.pctx.track_defs.clone();
				...
				if self.pctx.mute_solo_tracks.contains(track) == (self.pctx.mute_solo == MuteSolo::Mute) {
					...
					match spec {
						TrackDef::Instrument(structure) => {
							Some(self.build_nodes_by_mml(track.as_str(), structure, mml, /* &pctx, */ &seq_tag, /* &mut nodes, */ submachine_idx,
						...
						TrackDef::Groove(structure) => {
							let groovy_timer = self.build_nodes_by_mml(track.as_str(), structure, mml, /* &pctx, */ &seq_tag, /* &mut nodes, */ MACHINE_MAIN,
	...
	// dbg!(&domain_hints);return Ok(());
		...
		if ! no_play {
			...
			let master_node = {
				let master = self.add_mixer(&output_nodes, machine_out) ?;
	...
	fn add_mixer(&mut self, output_nodes: &HashMap::<String, NodeId>, machine_out: MachineIndex) -> ModdlResult<NodeId> {
	...
	fn add_register(&mut self, /* reg_inits: &mut RegisterSettingsTree, nodes: &mut AllNodes, */ machine: MachineIndex, path: QualifiedLabel, reg_key: impl Into<String>, value: f32, domain_hint: Option<DomainHint>) -> NodeId {
		add_register(&mut self.reg_inits, &mut self.nodes, machine, path, reg_key, value, domain_hint)
		// // reg_inits.push((tag.clone(), value));
		// self.reg_inits.add_settings(&path, reg_key, value, domain_hint);

		// // TODO QLabel をどこまで QLabel のまま持っていくか？ とりあえずここで String に変換しておくが
		// self.nodes.add_node_with_tag(machine, path.to_string(), Box::new(Var::new(value)))
	}

	fn build_nodes_by_mml<'b>(&mut self, track: &str, instrm_def: &ModuleDef, mml: &'b str, /* pctx: &PlayerContext, */ seq_tag: &String, /* nodes: &mut AllNodes, */ submachine_idx: MachineIndex, placeholders: &mut PlaceholderStack, override_input: Option<NodeId>/* ,
		...
		let mut input = match override_input {
			Some(input) => input,
			None => self.add_register(/* reg_inits, nodes, */ submachine_idx, freq_qual_tag.clone(), VAR_DEFAULT_KEY, 0f32, None),
		};
		if features.contains(&Feature::Detune) {
			...
			let detune = self.add_register(/* reg_inits, nodes, */ submachine_idx, QualifiedLabel::new(vec![track.to_string()], "#detune".to_string()), VAR_DEFAULT_KEY, DETUNE_INIT,
					Some(DomainHint::range_including_both_ends(-100f32, 100f32)));
		...
		// let instrm = self.build_instrument(track, instrm_def, /* nodes, */ submachine_idx, input, placeholders, &label_defaults, /* pctx.use_default_labels, */ /* &mut inits, */ /* reg_inits */) ?;
		// let instrm = self.build_instrument(track, instrm_def, submachine_idx, input, placeholders, &label_defaults) ?;
		let instrm = InstrumentBuilder::new(track, submachine_idx, placeholders, &label_defaults, &mut self.reg_inits, &mut self.nodes, self.pctx.use_default_labels).build(instrm_def, input) ?;
		...
		if features.contains(&Feature::Velocity) {
			// TODO タグ名は feature requirements として generate_sequences の際に受け取る
			let vel = self.add_register(/* reg_inits, nodes, */ submachine_idx, QualifiedLabel::new(vec![track], "#velocity"), VAR_DEFAULT_KEY, VELOCITY_INIT,
		...
		if features.contains(&Feature::Volume) {
			// TODO タグ名は feature requirements として generate_sequences の際に受け取る
			let vol = self.add_register(/* reg_inits, nodes, */ submachine_idx, QualifiedLabel::new(vec![track], "#volume"), VAR_DEFAULT_KEY, VOLUME_INIT,
...
impl <'a> InstrumentBuilder<'a> {
	...
	fn build(
		...
		self.build_rec(/* track, */ instrm_def, /* nodes, */ /* submachine_idx, */ freq, false)
	}
	...
	fn build_rec_full(&mut self, strukt: &ModuleDef, input: NodeId, inside_label_guard: bool, default_tag: Option<QualifiedLabel>, domain_hint: Option<&DomainHint>) -> ModdlResult<NodeId> {
		...
		let mut make_node_args = |args: &HashMap<String, Value>, def: &NodeDef/* , label: String */|
			...
			for NodeArgSpec { name, channels, default } in specs {
				...
				*/
				...
				let coerced_arg_node = match self.coerce_input(/* Some(track), nodes, submachine_idx, */ arg_node, channels) {
					Some(result) => result,
		...
		match strukt {
			...
			ModuleDef::NodeCreation { factory, args, label } => {
				...
				self.apply_input(/* Some(track), nodes, submachine_idx, */ &factory.node, &node_args, full_tag, input)
			}
	...
	fn apply_input(
		...
		match self.coerce_input(/* track, nodes, submachine_idx, */ input, fact.input_channels()) {
			Some(result) => {
...
// 	// });
	...
	// let machine_specs = nodes.result();
...
// 			// $const_tag は、直下が定数値（ノードの種類としては Var）であった場合に付与するタグ
			// 5 引数版
// 			($strukt: expr, $input: expr, $inside_label_guard: expr, $const_tag: expr, $domain_hint: expr) => {
				// visit_struct(
				// 	track,
				// 	$strukt,
				// 	nodes,
				// 	submachine_idx,
				// 	$input,
				// 	/* Some( */$const_tag/* ) */, <- default_tag: Option<QualifiedLabel>,
				// 	placeholders,
				// 	label_defaults,
				// 	use_default_labels,
				// 	reg_inits,
				...
				// ) };
			// 3 引数版
// 			($strukt: expr, $input: expr, $inside_label_guard: expr) => {
				// visit_struct(
				// 	track,
				// 	$strukt,
				// 	nodes,
				// 	submachine_idx,
				// 	$input,
				// 	None, // ★const_tag <- default_tag: Option<QualifiedLabel>,
				// 	placeholders,
				// 	label_defaults,
				// 	use_default_labels,
				// 	reg_inits,
