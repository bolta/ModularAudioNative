// 演奏に使うオーディオグラフの構築を行うモジュール

use super::{
	common::make_seq_tag, error::*, evaluator::*, import::ImportCache, io::Io, player_context::{MuteSolo, TrackDef}, player_option::*, value::*
};
use crate::{
...
use std::{
	borrow::Borrow, cell::RefCell, collections::hash_map::HashMap, rc::Rc, 
};

const MACHINE_MAIN: MachineIndex = MachineIndex(0usize);
pub const VAR_DEFAULT_KEY: &str = "value"; // TODO VarFactory を設けてそこから取るようにする
const TAG_FREQ: &str = "#freq";

type PlaceholderStack = Stack<HashMap<String, NodeId>>;
...
impl <'a> GraphBuilder<'a> {
	pub fn new(pctx: &'a PlayerContext, imports: &'a mut ImportCache<'a>) -> Self {
		Self {
			pctx,
			imports,
...
fn collect_label_defaults(instrm_def: &ModuleDef, track: &str, use_default_labels: bool, result: &mut HashMap<QualifiedLabel, String>) {
		...
		match strukt {
			...
			ModuleDef::NodeCreation { factory, args, label } => {
				...
				if let Some(tag) = &full_tag {
					for (_key, value) in factory.node.initial_values() {
